#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Markdown -> semantic HTML converter for the ʿAqīdah-Ijāzah-Prüfung.
Adapted from build_exam.py: reuses the book's Islamic layout (same design
tokens) plus the exam-specific components (cover, Name/Datum bar,
Prüfungsordnung, Hadith, Hinweise, Bewertungsschlüssel, point badges,
situational-analysis dialogue boxes)."""
import re, html

BASE_CSS = "/home/user/DBZ-App/aqidah_lektionen_deutsch_final.css"
...
def build_css(exam_css_path):
    base = open(BASE_CSS, encoding='utf-8').read()
    add = r'''

/* =========================================================================
   IJĀZAH-PRÜFUNG (EXAM) — zusätzliche Komponenten, gleiche Farbwelt wie das Buch
   ========================================================================= */
@page {
  @top-center {
    content: "ʿAqīdah-Ijāzah-Prüfung · Durūs Muhimmah li ʿĀmmat il-Ummah fil-ʿAqīdah";
...
'''
    open(exam_css_path, 'w', encoding='utf-8').write(base + add)

def build(src_path, out_html, out_css, css_link, variant_label):
...
<section class="content" style="page-break-after:always">
  ...
  <div class="info-note">
    <ul>
      <li><span class="k">Antwortform:</span> Die Prüfung enthält nur Fragen (Angaben) — <strong>keine</strong> Lücken zum Ausfüllen. Schreibe alle Antworten auf ein separates Blatt oder tippe sie leserlich auf iPad/PC ab (mit der jeweiligen Fragenummer).</li>
      ...
      <li><span class="k">Qurʾān &amp; Ḥadīth:</span> Es müssen <strong>keine</strong> Āyāt wörtlich und <strong>keine</strong> Ḥadīth-Nummern auswendig angegeben werden. Auch die Beweise, die der Autor im Text anführt, müssen nicht wörtlich zitiert werden — der <strong>grobe Inhalt</strong> (was geschah, was der Vers/Ḥadīth sinngemäß aussagt) genügt und ist erwünscht.</li>
      <li><span class="k">Was gewusst werden muss:</span> die konkreten Inhalte <strong>aus Buch und Unterricht</strong> — Definitionen, Listen (z.&nbsp;B. die Vernichter), Beispiele, Einteilungen und Prinzipien, so wie sie im Text und in den Lektionen behandelt wurden. Es wird gezielt <strong>dieser Stoff</strong> geprüft, nicht allgemeines ʿAqīdah-Wissen von anderswo.</li>
    </ul>
  </div>
...
'''
    ...
    doc = ('<!DOCTYPE html>\n<html lang="de" dir="ltr">\n<head>\n'
           '<meta charset="utf-8">\n'
           '<title>ʿAqīdah-Ijāzah-Prüfung (%s)</title>\n'
           '<link rel="stylesheet" href="%s">\n'
           '</head>\n<body>\n'
           % (variant_label, css_link)
           + cover + info +
           '\n<main class="content">\n' + body_html + '\n</main>\n</body>\n</html>')

    open(out_html, 'w', encoding='utf-8').write(doc)
    print("Wrote", out_html, "(%d bytes)" % len(doc), "| total=%d pass>=%d" % (total, pass_pts))

if __name__ == '__main__':
    build(
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_hauptpruefung.md",
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_hauptpruefung.html",
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_hauptpruefung.css",
        "aqidah_ijazah_pruefung_hauptpruefung.css",
        "HAUPTPRÜFUNG",
    )
    build(
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_vorbereitung.md",
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_vorbereitung.html",
        "/home/user/DBZ-App/aqidah_ijazah_pruefung_vorbereitung.css",
        "aqidah_ijazah_pruefung_vorbereitung.css",
